...
public class StudentService {
    ...
    public List<Course> getAvailbleCourses(int studentID){
        ...
        List<Course> allCourses = new CourseRepository().loadCourses();

        List<Course> available = new ArrayList<>();

        for (Course c : allCourses) {
        if (!target.getEnrolledCourses().contains(c.getId())) {
            available.add(c);
          }
        }
    ...
    public boolean enroll(int studentID, int courseID){
        ...
        CourseService enroll=new CourseService();
        Course course = enroll.courseExist(courseID);
